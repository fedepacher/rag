...
`load_local` is called with `allow_dangerous_deserialization=True`. A FAISS index is a pickle, and `langchain-community >= 0.0.27` refuses to unpickle one unless the caller vouches for its origin. Vouching is correct here: the else-branch of the same method writes the file with `save_local` from the course documents, and `document_has_changed` gates the load on a SHA-256 of that same content. Nothing is ever downloaded. **The index now has a volume** (`faiss_index:/app/resources/faiss_index`, #33). It used to live in the container's writable layer, which is why the missing `allow_dangerous_deserialization` flag once looked intermittent: a first boot took the build branch and succeeded, a restart took the load branch and crashed.

Persisting the index is safe **only because** `get_index_hash` covers the embedding model as well as the chunk text. Before `6729bec` it hashed the text alone, so a stored index would have been reloaded with 384-dimensional GPT4All vectors against a 1024-dimensional bge-m3 model — retrieval degrading while looking healthy. The missing volume had been hiding that by wiping the index on every rebuild, which was luck rather than design; the volume removes the luck, and `tests/test_index_invalidation.py` pins the invalidation instead. Measured: loading from the volume takes ~5 s against ~14.5 min to re-embed 148 chunks.
...
- **Embeddings no longer touch the public internet, and that removed a gotcha rather than fixing it.** `GPT4AllEmbeddings()` used to call `list_models()` at startup, a bare `requests.get` on `https://gpt4all.io/models/models3.json` that raised on any non-200; GitHub's CDN returned an intermittent `503 Backend.max_conn reached` and took the whole RAG service down three times in a row. A bounded retry (`build_gpt4all_embeddings`, `EMBEDDING_INIT_ATTEMPTS`) existed solely to survive it. `6729bec` replaced the model with `bge-m3` over the Ollama server already in the container, so the retry **and** the internet dependency are both gone — `build_ollama_embeddings()` in `rag/main.py` reaches nothing outside the box. Do not reintroduce a startup retry for embeddings; there is no longer a transient failure to retry.
- **Three models now, not two.** `entrypoint.sh` starts Ollama, waits for readiness, downloads `llama3.1:8b-instruct-q4_K_M`, `phi3.5:3.8b-mini-instruct-q4_K_M` and `bge-m3` if missing, then runs `rag/main.py` — a cold start with an empty `ollama` volume pulls ~8.5 GB of weights and is slow. At runtime the loaded footprint is larger than the weights: ~5.6 GiB for Llama plus ~3.2 GiB for Phi at `num_ctx=6000`, so ~8.5–9 GiB combined on the 16 GB target box. `enable_crag=False` never builds the Phi client, which is why a classic/baseline run stays at the pre-CRAG footprint.
- **The API waits on a compose healthcheck, not on a timeout.** `database` has a `mysqladmin ping` healthcheck and `api`/`email` declare `depends_on: {database: {condition: service_healthy}}` (#37). The bare `depends_on` it replaced only waited for the container to *start*, and `wait-for-it.sh` then raced it with a 15-second timeout. When `database` came back attached to zero networks, `api` died with a DNS error naming `database` while `docker compose ps` showed the database as a healthy-looking `Up` — the actual cause named nowhere. `wait-for-it.sh` is still in the image and still runs; it is now a second line of defence rather than the only one.
